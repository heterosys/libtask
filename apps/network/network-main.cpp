...
using pkt_vec_t = task::vec_t<pkt_t, kN>;

void Network(task::mmap<pkt_vec_t> input, task::mmap<pkt_vec_t> output,
             uint64_t n);
...
int main(int argc, char *argv[]) {
  ...
  Network(task::mmap<pkt_t>(input).vectorized<kN>(),
          task::mmap<pkt_t>(output).vectorized<kN>(), n / kN);
