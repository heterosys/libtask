...
template <typename T, uint64_t S> class istreams;

template <typename T, uint64_t S> class ostreams;

namespace internal {

template <typename T> struct elem_t {
  T val;
  bool eot;
};
...
template <typename T> class lock_free_queue : public base_queue {
...
template <typename T> class locked_queue : public base_queue {
  size_t depth;
...
template <typename T> class basic_streams {
protected:
...
/// Provides consumer-side operations to a @c task::stream where it is used as
/// an @a input.
///
/// This class should only be used in task function parameters and should never
/// be instatiated directly.
template <typename T> class istream : virtual public internal::basic_stream<T> {
public:
...
private:
  ...
  template <typename U, uint64_t S> friend class istreams;
  template <typename U, uint64_t S, uint64_t N> friend class streams;
  istream(const internal::basic_stream<T> &base)
      : internal::basic_stream<T>(base) {}
};

/// Provides producer-side operations to a @c task::stream where it is used as
/// an @a output.
///
/// This class should only be used in task function parameters and should never
/// be instatiated directly.
template <typename T> class ostream : virtual public internal::basic_stream<T> {
public:
...
private:
  ...
  template <typename U, uint64_t S> friend class ostreams;
  template <typename U, uint64_t S, uint64_t N> friend class streams;
...
public:
  ...
  /// Constructs a @c task::stream.
  stream()
      : internal::basic_stream<T>(
            std::make_shared<internal::queue<internal::elem_t<T>>>(N)) {}

  /// Constructs a @c task::stream with the given name for debugging.
  ///
...
/// Alternative name of @c task::stream.
template <typename T, uint64_t depth> using channel = stream<T, depth>;

/// Provides consumer-side operations to an array of @c task::stream where they
...
public:
  /// Length of the @c task::stream array.
  constexpr static int length = S;

  /// References a @c task::stream in the array.
  ///
  /// @param pos Position of the array reference.
  /// @return    @c task::istream referenced in the array.
...
protected:
  ...
  template <typename U, uint64_t friend_length> friend class istreams;
...
private:
  ...
  template <uint64_t length> istreams<T, length> access() {
    CHECK_NOTNULL(this->ptr.get());
...
/// Provides producer-side operations to an array of @c task::stream where they
...
public:
  ...
  ///
  /// @param pos Position of the array reference.
  /// @return @c task::ostream referenced in the array.
...
protected:
  ...
  template <typename U, uint64_t friend_length> friend class ostreams;
...
private:
  ...
  template <uint64_t length> ostreams<T, length> access() {
    CHECK_NOTNULL(this->ptr.get());
...
/// Defines an array of @c task::stream.
template <typename T, uint64_t S, uint64_t N>
class streams : public internal::unbound_streams<T, S> {
public:
  /// Count of @c task::stream in the array.
  constexpr static int length = S;

  /// Depth of each @c task::stream in the array.
  constexpr static int depth = N;

  /// Constructs a @c task::streams array.
  streams()
  ...
  /// Constructs a @c task::streams array with the given base name for
  /// debugging.
  ///
  /// The actual name of each @c task::stream would be <tt>name[i]</tt>.
  ///
  ...
  ///
  /// @param pos Position of the array reference.
  /// @return @c task::stream referenced in the array.
...
private:
  ...
  template <uint64_t length> istreams<T, length> access_as_istreams() {
    CHECK_NOTNULL(this->ptr.get());
    istreams<T, length> result;
    result.ptr = std::make_shared<std::vector<internal::basic_stream<T>>>();
    result.ptr->reserve(length);
  ...
  template <uint64_t length> ostreams<T, length> access_as_ostreams() {
    CHECK_NOTNULL(this->ptr.get());
    ostreams<T, length> result;
    result.ptr = std::make_shared<std::vector<internal::basic_stream<T>>>();
    result.ptr->reserve(length);
...
/// Alternative name of @c task::streams.
template <typename T, uint64_t S, uint64_t N> using channels = streams<T, S, N>;
...
#define task_DEFINE_ACCESSER(io, reference)                                    \
  ...
  struct accessor<io##streams<T, param_length> reference,                      \
    ...
    static io##streams<T, param_length>                                        \
    access(streams<T, arg_length, depth> &arg) {                               \
  ...
  struct accessor<io##streams<T, param_length> reference,                      \
    ...
    static io##streams<T, param_length>                                        \
    access(io##streams<T, arg_length> &arg) {                                  \
