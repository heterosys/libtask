...
template <typename T, int N> struct vec_t : protected std::array<T, N> {
private:
...
public:
  ...
  template <typename U> explicit operator vec_t<U, N>() const {
    vec_t<U, N> result;
...
#define DEFINE_OP(op)                                                          \
  ...
  template <typename T2> vec_t<T, N> &operator op##=(const T2 &rhs) {          \
    _Pragma("HLS inline");                                                     \
    for (size_type i = 0; i < N; ++i) {                                        \
      _Pragma("HLS unroll");                                                   \
      set(i, get(i) op rhs);                                                   \
...
#define DEFINE_OP(op)                                                          \
  template <typename T2> vec_t<T, N> operator op(const vec_t<T2, N> &rhs) {    \
    _Pragma("HLS inline");                                                     \
    vec_t<T, N> result;                                                        \
    for (size_type i = 0; i < N; ++i) {                                        \
      _Pragma("HLS unroll");                                                   \
...
template <typename T, typename... Args> inline auto cat(T arg, Args... args) {
...
template <int N, typename T> vec_t<T, N> make_vec(T val) {
  vec_t<T, N> result;
  result.set(val);
  return result;
}
...
#define DEFINE_FUNC(func)                                                      \
  template <typename T, int N> vec_t<T, N> func(vec_t<T, N> vec) {             \
    _Pragma("HLS inline");                                                     \
    for (int i = 0; i < N; ++i) {                                              \
      _Pragma("HLS unroll");                                                   \
...
#define DEFINE_FUNC(func, op)                                                  \
  template <typename T> T func(const vec_t<T, 1> &vec) {                       \
    _Pragma("HLS inline");                                                     \
    return vec[0];                                                             \
  }                                                                            \
  template <typename T, int N> T func(const vec_t<T, N> &vec) {                \
    _Pragma("HLS inline");                                                     \
